...
function PowerAnalysis_Br(filename)

% Input Variable definitions - certain variables need to be changed for differing locations

Q_total = 10.694; %Total airflow [m3/s]
angle = 24; %angle of tilt of solar heater, [deg]
p_supply = 0.7; %Proportion of supply air that is fresh
Tr_glass = 0.83; %Transmittance of glass
L_wall = 21.3; %Length of wall (m)
...
CO2_limit = 3000; %Limit at which system goes into CO2 Exhaust mode
...
q_bees = 58000; %Energy given off by bees - building heat loss[W] 
...
CO2_bees_ppm = ((CO2_bees / ACH / 44.01) / ((dens_air / 0.02897)*Vol_bldg)) * 1000000; %ppm of CO2 generated per ACH
...
% Solar Air Heater - Internal Convection coefficient (doesn't change with hourly climate data)
...
h_int = (Nu_SAH * k_air) / D_h; %Internal SAH convection coefficient[W/m2K]
...
for i = 1:(n-1)
	...
	T_ext = Temp(i);
	Hum_ext = Hum(i);
	Ins(i) = (SolB(i)*sind(i_angle)) + (SolD(i)); %Solar insolation is the sum of direct and diffuse irradiation [kJ/m2/hr]
	...
	for j = 1:5 
		T_SAH_f = (q_SAH_tot / (Q_total * dens_air * cp_air)) + T_ext; %Calculating final temperature of SAH
	...
	if (T_ent(i) < T_desired)
		...
		Eff_HEx = (T_desired - T_ent(i)) / (T_w_in - T_ent(i));
		%HEx Eff.
		...
		for k = 1:ACH %air changes per hour
			CO2(i) = CO2(i) + (p_supply*CO2_ext) - (p_supply*CO2(i)) + CO2_bees_ppm;
		end 

		CO2_old = CO2(i);
		T(i) = T_desired;
		P_elec(i) = P_fans + P_f_pump; %Fans + pump
		P_gas(i) = P_boiler * 0.0000036; %Boiler is the source of natural gas usage, convert to GJ

		%Humidity Changes:
		for m = 1:ACH
			AHum(i) = (AHum_ent*p_supply) + AHum_old + (H2O_bees/(ACH*Vol_bldg)) - (p_supply*AHum_old);
			AHum_old = AHum(i);
		end 

	else 
		T_ent(i) = T_ext; %If entering temperature too high, open up solar heater bypass
		...
		for k = 1:ACH %air changes per hour
			if or((CO2(i) >= CO2_limit),(AHum_old > HumLim))
				...
				P_fans = P_fans + ((4*P_f_exh)/ACH); %Pump turned off
				AHum(i) = AHum_old - (p_supply*AHum_old) + (H2O_bees/(ACH*Vol_bldg));
				AHum_old = AHum(i);
			else 
				CO2(i) = CO2(i) + CO2_bees_ppm;
				T(i) = T(i) + (q_bees/(Q_total*(1-p_supply)*dens_air*cp_air*(3600/ACH)));
				P_fans = P_fans + (P_f_ret/ACH); %No pump in this mode
		...
		HumInt(i) = abs((AHum(i)*(273.15*T(i)))/(6.112*exp((17.67*T(i))/(T(i)+243.4))*21.674));
	end 

	%Cost calcs
	CostE(i) = (C_elec * P_elec(i)) / 100;
	CostNG(i) = C_NG * P_gas(i);
	fprintf('\n %i %.2f %.2f %.2f %.2f %.2f',i,HumInt(i),T(i),CO2(i),sum(CostE),sum(CostNG));
...
Data = vertcat(P_elec,CostE,P_gas,CostNG,T,CO2,HumInt);
Data = transpose(Data);

%Writing data to an excel file
excelfile = 'BrooksData.xlsx';
